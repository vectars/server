...
if [ -t 1 ] && [ -t 2 ]; then
  ...
  C_YELLOW=$'\033[33m'; C_RED=$'\033[31m'; C_BLUE=$'\033[34m'; C_WHITE=$'\033[37m'; C_DIM=$'\033[2m'
else
  C_RESET=''; C_BOLD=''; C_CYAN=''; C_GREEN=''; C_YELLOW=''; C_RED=''; C_BLUE=''; C_WHITE=''; C_DIM=''
fi
...
list_os_images(){
  echo >&2
  printf '%s\n' "$C_CYAN"'============================================================'"$C_RESET" >&2
  printf '  %s%sAVAILABLE OPERATING SYSTEMS%s\n' "$C_BOLD" "$C_WHITE" "$C_RESET" >&2
  printf '%s\n' "$C_CYAN"'============================================================'"$C_RESET" >&2
  printf '  %s01%s  %sUbuntu 24.04%s        images:ubuntu/24.04\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s02%s  %sUbuntu 22.04%s        images:ubuntu/22.04\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s03%s  %sDebian 13%s           images:debian/13\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s04%s  %sDebian 12%s           images:debian/12\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s05%s  %sAlpine Linux%s         images:alpine/3.22\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s06%s  %sRocky Linux 9%s       images:rockylinux/9\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s07%s  %sAlmaLinux 9%s          images:almalinux/9\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s08%s  %sFedora 42%s            images:fedora/42\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s09%s  %sArch Linux%s           images:archlinux\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s10%s  %sKali Linux%s           images:kali\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s11%s  %sCentOS Stream 9%s     images:centos/9-Stream\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s12%s  %sOracle Linux 9%s      images:oracle/9\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s13%s  %sAmazon Linux 2023%s    images:amazonlinux/2023\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s14%s  %sopenSUSE Leap 15.6%s  images:opensuse/leap/15.6\n' "$C_GREEN" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '  %s15%s  %sCustom LXD image%s     enter manually\n' "$C_YELLOW" "$C_RESET" "$C_WHITE" "$C_RESET" >&2
  printf '%s\n' "$C_CYAN"'------------------------------------------------------------'"$C_RESET" >&2
}
...
select_os(){
  local choice image
  list_os_images
  read -r -p "Select OS [01]: " choice
  choice="${choice:-1}"
  case "$choice" in
    1|01) image="images:ubuntu/24.04" ;;
    2|02) image="images:ubuntu/22.04" ;;
    3|03) image="images:debian/13" ;;
    4|04) image="images:debian/12" ;;
    5|05) image="images:alpine/3.22" ;;
    6|06) image="images:rockylinux/9" ;;
    7|07) image="images:almalinux/9" ;;
    8|08) image="images:fedora/42" ;;
    9|09) image="images:archlinux" ;;
    10) image="images:kali" ;;
    11) image="images:centos/9-Stream" ;;
    12) image="images:oracle/9" ;;
    13) image="images:amazonlinux/2023" ;;
    14) image="images:opensuse/leap/15.6" ;;
    15)
      read -r -p "Custom LXD image (remote:image or fingerprint): " image
      ;;
    *)
      warn "Invalid OS selection. Using Ubuntu 24.04."
...
guest_apt_repair(){
  ...
  lxc exec "$n" -- bash -lc '
    ...
    # Configure DNS for common Linux guests without assuming Ubuntu.
    mkdir -p /etc/systemd/resolved.conf.d 2>/dev/null || true
    cat >/etc/systemd/resolved.conf.d/90-nkvm-network.conf 2>/dev/null <<EOF || true
[Resolve]
DNS=1.1.1.1 8.8.8.8
FallbackDNS=9.9.9.9 1.0.0.1
DNSStubListener=yes
EOF
    ...
    # Use the package manager that belongs to the selected guest OS.
...
EOF
      ...
      for i in 1 2 3; do dnf -y makecache && exit 0; sleep "$i"; done
      exit 1
    elif command -v yum >/dev/null 2>&1; then
      for i in 1 2 3; do yum makecache && exit 0; sleep "$i"; done
      exit 1
    elif command -v apk >/dev/null 2>&1; then
      for i in 1 2 3; do apk update && exit 0; sleep "$i"; done
      exit 1
    elif command -v zypper >/dev/null 2>&1; then
      for i in 1 2 3; do zypper --non-interactive refresh && exit 0; sleep "$i"; done
      exit 1
    fi
...
ensure_guest_ssh(){
  ...
  lxc exec "$n" -- bash -lc '
    ...
    if command -v apt-get >/dev/null 2>&1; then
      ...
      dnf install -y openssh-server >/dev/null 2>&1 || exit 1
    elif command -v yum >/dev/null 2>&1; then
      yum install -y openssh-server >/dev/null 2>&1 || exit 1
    elif command -v apk >/dev/null 2>&1; then
      apk add --no-cache openssh >/dev/null 2>&1 || exit 1
    elif command -v zypper >/dev/null 2>&1; then
      zypper --non-interactive install openssh-server >/dev/null 2>&1 || exit 1
    else
    ...
    mkdir -p /run/sshd
    ss -lntp 2>/dev/null | grep -q ':22 ' || netstat -lntp 2>/dev/null | grep -q ':22 ' || exit 1
  ' >>"$LOG" 2>&1 || {
    warn "$n SSH server could not be installed automatically. Port forwarding requires SSH on guest port 22."
    return 1
  }
...
diagnostics(){
  detect_uplink || true
  ui_title "NKVM DIAGNOSTICS ${VERSION}"
...
menu(){
  while true; do
    status
    echo
    printf '  %s%sVPS MANAGEMENT%s\n' "$C_BOLD" "$C_CYAN" "$C_RESET"
    echo "  1) Create VPS"
    echo "  2) Delete VPS"
    echo "  3) Repair VPS"
    echo "  4) VPS shell"
    echo "  5) Show diagnostics"
    echo "  6) Edit VPS specifications"
    echo "  7) Exit"
    echo
    printf '  %s%sMade by Vectar%s\n' "$C_DIM" "$C_WHITE" "$C_RESET"
...
main(){
  ...
  ui_title "NKVM VPS MANAGER ${VERSION}"
