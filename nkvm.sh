...
ui_header(){
  echo
  ui_line
  printf '%s\n' "${C_BOLD}${C_CYAN}                    NKVM VPS MANAGER${C_RESET}"
  printf '%s\n' "${C_DIM}                 Version ${VERSION}${C_RESET}"
  printf '%s\n' "${C_BOLD}                       Made by Vectar${C_RESET}"
  ui_line
}
ui_footer(){
  ui_line
  printf '%s\n' "${C_DIM}                         Made by Vectar${C_RESET}"
}
BR_PREFIX="nkvbr"
SUBNET_PREFIX="10.77"
BRIDGE_NAME=""
...
ensure_forwarding(){
  ...
  cat >/etc/sysctl.d/99-nkvm-lxd.conf <<EOF
...
guest_dns_repair(){
  ...
  lxc exec "$n" -- bash -lc '
    ...
    cat >/etc/systemd/resolved.conf.d/90-nkvm.conf <<EOF
[Resolve]
DNS=1.1.1.1 8.8.8.8
...
guest_apt_repair(){
  ...
  lxc exec "$n" -- bash -lc '
    ...
    cat >/etc/apt/apt.conf.d/80-nkvm-network <<EOF
...
remove_port_forward(){
  local n="$1" host_port="$2" guest_ip host_ip
  validate_port "$host_port" || return 0
  guest_ip="$(lxc exec "$n" -- ip -4 -o addr show dev eth0 2>/dev/null | awk '{print $4}' | cut -d/ -f1 | head -1)"
  host_ip="$(host_ipv4)"

  if [ -n "$guest_ip" ]; then
    while iptables -t nat -C PREROUTING -i "$UPLINK" -p tcp --dport "$host_port" -j DNAT --to-destination "${guest_ip}:22" >/dev/null 2>&1; do
      iptables -t nat -D PREROUTING -i "$UPLINK" -p tcp --dport "$host_port" -j DNAT --to-destination "${guest_ip}:22" >/dev/null 2>&1 || break
    done
    while iptables -C FORWARD -i "$UPLINK" -o "$BRIDGE_NAME" -p tcp -d "$guest_ip" --dport 22 -j ACCEPT >/dev/null 2>&1; do
      iptables -D FORWARD -i "$UPLINK" -o "$BRIDGE_NAME" -p tcp -d "$guest_ip" --dport 22 -j ACCEPT >/dev/null 2>&1 || break
    done
    while iptables -C FORWARD -i "$BRIDGE_NAME" -o "$UPLINK" -p tcp -s "$guest_ip" --sport 22 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT >/dev/null 2>&1; do
      iptables -D FORWARD -i "$BRIDGE_NAME" -o "$UPLINK" -p tcp -s "$guest_ip" --sport 22 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT >/dev/null 2>&1 || break
    done
  fi

  if [ -n "$host_ip" ]; then
    while iptables -t nat -C OUTPUT -d "$host_ip" -p tcp --dport "$host_port" -j DNAT --to-destination "${guest_ip}:22" >/dev/null 2>&1; do
      iptables -t nat -D OUTPUT -d "$host_ip" -p tcp --dport "$host_port" -j DNAT --to-destination "${guest_ip}:22" >/dev/null 2>&1 || break
    done
  fi
...
edit_vps(){
  local n="$1" current_ram current_cpu current_disk current_port new_ram new_cpu new_disk new_port
  lxc info "$n" >/dev/null 2>&1 || { err "$n does not exist."; return 1; }

  current_ram="$(lxc config get "$n" limits.memory 2>/dev/null || true)"
  current_cpu="$(lxc config get "$n" limits.cpu 2>/dev/null || true)"
  current_disk="$(lxc config device get "$n" root size 2>/dev/null || true)"
  current_port="$(awk -v n="$n" '$2 == n {print $1; exit}' "$PORT_DB" 2>/dev/null || true)"

  echo
  echo "Editing specifications for: $n"
  echo "Press Enter to keep the current value."
  echo
  read -r -p "RAM [${current_ram:-2GiB}]: " new_ram
  new_ram="${new_ram:-${current_ram:-2GiB}}"
  read -r -p "CPU [${current_cpu:-1}]: " new_cpu
  new_cpu="${new_cpu:-${current_cpu:-1}}"
  read -r -p "Disk [${current_disk:-5GiB}]: " new_disk
  new_disk="${new_disk:-${current_disk:-5GiB}}"
  read -r -p "Host SSH port [${current_port:-2201}]: " new_port
  new_port="${new_port:-${current_port:-2201}}"

  validate_port "$new_port" || { err "Invalid host port: $new_port (use 1-65535)."; return 1; }
  if awk -v p="$new_port" -v n="$n" '$1 == p && $2 != n {found=1} END {exit !found}' "$PORT_DB"; then
    err "Host port $new_port is already assigned to another VPS."
    return 1
  fi

  info "Applying VPS specifications..."
  lxc config set "$n" limits.memory "$new_ram" >>"$LOG" 2>&1 || { err "Could not set RAM."; return 1; }
  lxc config set "$n" limits.cpu "$new_cpu" >>"$LOG" 2>&1 || { err "Could not set CPU."; return 1; }
  lxc config device set "$n" root size "$new_disk" >>"$LOG" 2>&1 || { err "Could not set disk size."; return 1; }

  if [ -n "$current_port" ] && [ "$current_port" != "$new_port" ]; then
    remove_port_forward "$n" "$current_port"
  fi

  ensure_guest_ssh "$n" || true
  ensure_port_forward "$n" "$new_port" || return 1
  ok "Specifications updated: $n | RAM=$new_ram | CPU=$new_cpu | Disk=$new_disk | SSH port=$new_port"
}
...
diagnostics(){
  detect_uplink || true
  ui_header
  printf '%s\n' "${C_BOLD}SYSTEM DIAGNOSTICS${C_RESET}"
  ui_line
...
status(){
  ui_header
  printf '%s\n' "${C_BOLD}${C_CYAN}VPS OVERVIEW${C_RESET}"
  ui_line
  lxc list || true
  echo
  printf '%-18s %-10s %-16s %-10s %-6s %-10s %-8s\n' NAME STATE IPV4 RAM CPU DISK SSHPORT
  printf '%-18s %-10s %-16s %-10s %-6s %-10s %-8s\n' "------------------" "----------" "----------------" "----------" "------" "----------" "--------"
  while read -r n; do
    [ -n "$n" ] || continue
    local state ip ram cpu disk port
    ...
    port="$(awk -v n="$n" '$2 == n {print $1; exit}' "$PORT_DB" 2>/dev/null || true)"
    printf '%-18s %-10s %-16s %-10s %-6s %-10s %-8s\n' "$n" "$state" "${ip:-pending}" "${ram:--}" "${cpu:--}" "${disk:--}" "${port:--}"
  done < <(lxc list -c n --format csv 2>/dev/null)
}
...
menu(){
  while true; do
    ...
    printf '  %s1)%s Create VPS\n' "$C_GREEN" "$C_RESET"
    printf '  %s2)%s Delete VPS\n' "$C_RED" "$C_RESET"
    printf '  %s3)%s Repair VPS\n' "$C_YELLOW" "$C_RESET"
    printf '  %s4)%s VPS shell\n' "$C_BLUE" "$C_RESET"
    printf '  %s5)%s Show diagnostics\n' "$C_CYAN" "$C_RESET"
    printf '  %s6)%s Edit VPS specifications\n' "$C_GREEN" "$C_RESET"
    printf '  %s7)%s Exit\n' "$C_DIM" "$C_RESET"
    echo
    read -r -p "Select [7]: " choice
    case "${choice:-7}" in
      1)
...
main(){
  ...
  ui_header
  printf '%s\n' "${C_DIM}NKVM • Lightweight LXD VPS management${C_RESET}"
  printf '%s\n' "${C_DIM}Network, NAT, SSH forwarding, UFW and resource controls${C_RESET}"
  echo
